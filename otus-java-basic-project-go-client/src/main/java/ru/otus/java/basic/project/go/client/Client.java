package ru.otus.java.basic.project.go.client;

import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;
import ru.otus.java.basic.project.go.api.context.ChallengeContext;
import ru.otus.java.basic.project.go.api.context.Context;
import ru.otus.java.basic.project.go.api.context.GameContext;
import ru.otus.java.basic.project.go.api.enums.ChallengeResponse;
import ru.otus.java.basic.project.go.api.enums.MoveType;
...
import ru.otus.java.basic.project.go.api.messages.client.*;
import ru.otus.java.basic.project.go.api.messages.server.*;
import ru.otus.java.basic.project.go.client.exceptions.ApplicationException;
import ru.otus.java.basic.project.go.client.exceptions.ChallengeCancelledException;
import ru.otus.java.basic.project.go.client.exceptions.ServerError;
import ru.otus.java.basic.project.go.client.windows.*;
...
public class Client implements AutoCloseable {
    ...
    public CompletableFuture<Void> connectAndLoginAsync(String hostPort, String name, String password, boolean register) {
        CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
            try {
                ...
                serverConnection = new ServerConnection(hostPort);
        ...
        return future.thenCompose((_) -> loginAsync(name, password, register));
    }
    ...
    public CompletableFuture<Void> loginAsync(String name, String password, boolean register) {
        ...
        context.setListener(LoginConfirmedServerMessage.class, (message) -> {
            this.name = name;
    ...
    public CompletableFuture<Collection<String>> updateClientsListAsync() {
        ...
        Context context = new Context(Context.getNewId());
        context.setListener(ClientListServerMessage.class, (message) -> {
    ...
    public CompletableFuture<ChallengeResponse> challengeAsync(String client) {
        ...
        challengeContext.setListener(ErrorServerMessage.class, (message) -> {
            challengeFuture.completeExceptionally(new ExecutionException("Challenge Error",
                    new ServerError(message.getErrorMessage())));
        });
        challengeContext.setListener(ChallengeResponseServerMessage.class,
                (message) -> challengeFuture.complete(message.getResponse()));
        serverConnection.addContext(challengeContext);
    ...
    public CompletableFuture<Void> respondChallengeAsync(ChallengeContext context, ChallengeResponse response) {
        ...
        CompletableFuture<Void> future = new CompletableFuture<>();
        context.setListener(ErrorServerMessage.class, (message) -> {
            future.completeExceptionally(new ExecutionException("Challenge Error",
                    new ServerError(message.getErrorMessage())));
        });
    ...
    public CompletableFuture<GameContext> requestGameInfoAsync() {
        ...
        context.setListener(ErrorServerMessage.class, (message) -> {
            future.completeExceptionally(new ExecutionException("Game Error",
                    new ServerError(message.getErrorMessage())));
        });
        serverConnection.addContext(context);
        submitFutureSend(future, new RequestGameInfoClientMessage(context.getId()));
        return future.whenComplete((result, e) -> serverConnection.removeContext(context));
    }
